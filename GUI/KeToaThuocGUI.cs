...
{
    public partial class KeToaThuocGUI : Form
    {
        ...
        private long maBN;
        private long tongTienThuoc = 0;

        public KeToaThuocGUI(long maBN)
        {
            InitializeComponent();
            ...
            lbTongTienThuoc.Text = $"{tongTienThuoc:N0} VND";
        ...
        private void LoadKhoThuoc()
        {
            ...
            List<KhoThuocDTO> khoThuocList = khoThuocBLL.GetAll();
        ...
        private void TimKiemThuoc(string keyword)
        {
            try
            {
                ...
                List<KhoThuocDTO> khoThuocList = khoThuocBLL.TimKiemThuoc(keyword);
        ...
        private void btnThemThuocVaoToa_Click(object sender, EventArgs e)
        {
            if (tableKhoThuoc.SelectedRows.Count > 0)
            {
                ...
                if (int.TryParse(input, out int soLuong) && soLuong > 0 && soLuong <= soLuongTon)
                {
                    ...
                    // Duyệt qua các dòng để kiểm tra trùng lặp
                    foreach (DataRow row in dt.Rows)
                    {
                        long existingId = Convert.ToInt64(row["ID"]);
                        if (existingId == id)
                        {
                            // Nếu thuốc đã có, cộng thêm số lượng vào dòng hiện tại
                            int existingQuantity = Convert.ToInt32(row["SoLuong"]);
                            row["SoLuong"] = existingQuantity + soLuong;

                            // Tính lại tổng tiền
                            row["TongTien"] = Convert.ToDouble(row["DonGia"]) * Convert.ToInt32(row["SoLuong"]);
                    ...
                    // Nếu thuốc chưa có, thêm mới vào DataTable
                    if (!isExisting)
                    {
                    ...
                    // Cập nhật tổng tiền
                    lbTongTienThuoc.Text = $"{tongTienThuoc += (long)donGia * soLuong:N0} VND";
        ...
        //                lbTongTienThuoc.Text = $"{tongTienThuoc+= (long)tongTien:N0} VND";
        //            }
